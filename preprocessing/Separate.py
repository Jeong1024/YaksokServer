...
class Separate():
    def __init__(self, config):
        ...
        self.open_path = config['filter_path']
    ...
    def separate(self, dir_path, x):
        ...
        for filename in filenames:
            ...
            with Image.open(full_filename) as image:
                if i % 10 < 7:
                    training_directory = dir_path + 'training/' + '/' + x
                    shutil.copyfile(full_filename, training_directory + '/' + filename)
                    
                elif i % 10 >= 7 and i % 10 < 8:
                    validation_directory = dir_path + 'testing/' + '/' + x
                    shutil.copyfile(full_filename, validation_directory+ '/' + filename)
                    
                else:
                    testing_directory = dir_path + 'validation/' + '/' + x
                    shutil.copyfile(full_filename, testing_directory+ '/' + filename)
            i = i + 1
